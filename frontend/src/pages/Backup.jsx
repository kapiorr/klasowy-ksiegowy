...
export default function Backup() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {/* Eksport */}
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-sage-100 dark:border-gray-700 p-6 mb-4">
        <div>
          <div className="w-10 h-10 bg-sage-100 dark:bg-gray-700 rounded-xl flex items-center justify-center text-xl flex-shrink-0">⬇</div>
          <div className="flex-1">
            <h2 className="font-display font-700 text-ink mb-1">Eksport danych</h2>
            <p className="font-body text-sm text-sage-600 mb-4">
              Pobiera plik JSON z wszystkimi danymi: uczniami, składkami, wpłatami, wypłatami
              i kontami użytkowników (hasła są zahashowane — bezpieczne).
            </p>
            <button onClick={handleDownload} disabled={downloading}
              className="bg-ink text-white font-display font-600 px-5 py-2.5 rounded-xl hover:bg-sage-700 disabled:opacity-50 transition-colors">
              {downloading ? 'Pobieranie...' : '⬇ Pobierz backup'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-sage-100 dark:border-gray-700 p-6">
        <div>
          <div className="w-10 h-10 bg-rose-100 rounded-xl flex items-center justify-center text-xl flex-shrink-0">⬆</div>
          <div className="flex-1">
            <h2 className="font-display font-700 text-ink mb-1">Import / Przywracanie</h2>
            <p className="font-body text-sm text-sage-600 mb-1">
              Wgrywa backup i <strong className="text-rose-500">całkowicie zastępuje</strong> aktualne dane.
              Tej operacji nie można cofnąć.
            </p>
            <p className="font-body text-xs text-sage-400 mb-4">
              Akceptowane pliki: JSON wygenerowany przez funkcję eksportu powyżej.
            </p>

            {restoreState === 'idle' && (
              <label className="cursor-pointer inline-block">
                <span className="border-2 border-dashed border-rose-200 text-rose-400 hover:border-rose-400 hover:text-rose-500 font-body text-sm px-5 py-2.5 rounded-xl transition-colors inline-block">
            ...
            {restoreState === 'confirm' && (
              <div className="space-y-3">
                <div className="bg-rose-50 border border-rose-200 rounded-xl px-4 py-3">
                  <div className="font-body text-sm font-500 text-rose-700">⚠ Uwaga — operacja nieodwracalna</div>
                  <div className="font-body text-xs text-rose-600 mt-1">
                    Plik: <strong>{plik?.name}</strong><br/>
                    Wszystkie aktualne dane zostaną usunięte i zastąpione danymi z backupu.
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <button onClick={handleRestore}
                    className="flex-1 bg-rose-500 text-white rounded-xl py-2.5 font-display font-600 hover:bg-rose-600">
                    Tak, wgraj backup
                  </button>
                </div>
              </div>
            ...
            {restoreState === 'loading' && (
              <div className="font-body text-sm text-sage-600 py-2">⏳ Wgrywanie danych...</div>
            )}

            {restoreState === 'done' && (
              <div className="bg-sage-100 border border-sage-200 rounded-xl px-4 py-3 font-body text-sm text-sage-700">
                ✓ {restoreMsg}
              </div>
            )}
            ...
            {restoreState === 'error' && (
              <div className="space-y-3">
                <div className="bg-rose-50 border border-rose-200 rounded-xl px-4 py-3 font-body text-sm text-rose-600">
                  ✗ Błąd: {restoreMsg}
                </div>
                <button onClick={resetRestore}
                  className="text-sm font-body text-sage-600 underline">
                  Spróbuj ponownie
                </button>
              </div>
            )}
